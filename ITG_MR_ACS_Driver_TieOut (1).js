    ...
    function (search, config, runtime, email, file) {
        ...
        function GetInputData() {
            try {    
                ...
                //Load Search 1 - TLG Agent Commission Statement Driver Search
                var driverSearch = search.load({ id: FILE_CONST.SCRIPT_PARAM.ACS_DRIVER_SEARCH });
                searchType = 'DRIVER_SEARCH';
                ...
                log.debug('driverAgentCodes', 'GETINPUT driverAgentCodes: '+driverAgentCodes);
               
    
    
                //RUN SEARCH 2 and 3 and get Agent Codes
                //Search 2 - [New] ACS PDF Section Summary By Agent By Section
                var agentCodesBySection = [];
    

                //Load Search 2- ACS PDF Section Summary By Agent By Section Search
                var acsPDFSearch = search.load({ id: FILE_CONST.SCRIPT_PARAM.ACS_PDF_SEC_SUMM_SEARCH });
                searchType = 'ACS_PDF_SEARCH';
                var acsPDFSearchResults = getResults(acsPDFSearch, searchType);
                log.debug('acsPDFSearchResults', 'GETINPUT acsPDFSearchResults: '+JSON.stringify(acsPDFSearchResults));
               
    
                
                var stringifyVals = JSON.stringify(acsPDFSearchResults);
                var cleanStr = stringifyVals.replace(/[()]/g, '');
                var newObj = JSON.parse(cleanStr);  
    
                for(var j=0; acsPDFSearchResults && j < acsPDFSearchResults.length; j++){               
                ...
                //Load Search 3- ACS PDF Supplier Comm Section Summary By Agent By Section Search
                var suppCommSearch = search.load({ id: FILE_CONST.SCRIPT_PARAM.ACS_SUPP_COMM_SUMM_SEARCH });
                searchType = 'SUPP_COMM_SEARCH';
                var suppCommSearchResults = getResults(suppCommSearch, searchType);
                log.debug('suppCommSearchResults', 'GETINPUT suppCommSearchResults: '+JSON.stringify(suppCommSearchResults));
                ...
                log.debug('agentCodesBySection', 'GETINPUT agentCodesBySection: '+agentCodesBySection);
     
    
                var missingAgentCodes = [];
                for(var i=0; i < agentCodesBySection.length; i++){
                    if(driverAgentCodes.indexOf(agentCodesBySection[i]) == -1){
                        if(missingAgentCodes.indexOf(agentCodesBySection[i]) == -1){ 
                            missingAgentCodes.push(agentCodesBySection[i]); 
                        }
                    }
                }
                
                log.debug('missingAgentCodes', 'GETINPUT missingAgentCodes: '+missingAgentCodes);

                if(missingAgentCodes  == '' || missingAgentCodes == null || missingAgentCodes == undefined || missingAgentCodes == "- None -"){
                    ...
                    email.send({
                        ...
                        subject: 'No Missing Agent Codes',
                        body: 'All the Agent Codes has Driver records created.'
                    });

                    
                }

                return missingAgentCodes;

            } catch (ex) {
                log.error('Error in getInputData::', JSON.stringify(ex));
                var configRecObj = config.load({ type: config.Type.COMPANY_PREFERENCES });
                var emailFrom = configRecObj.getValue('custscript_error_email_author');
                log.debug('emailFrom: ', emailFrom);
        ...
        function getResults(searchLoad, searchType){
            try{
                var driverSubsidiary = FILE_CONST.SCRIPT_PARAM.SUB;
                //log.debug('SUBS ID', 'SubsID: '+driverSubsidiary);
               
                var periodID = FILE_CONST.SCRIPT_PARAM.PERIOD;
                //log.debug('periodID', 'PeriodID: '+periodID);
                ...
                if(searchType == 'DRIVER_SEARCH' || searchType == 'INACTIV_VEND_DRIVER_SEARCH'){                
                    ...
                    var filterOne = search.createFilter({ 
                            name: 'custrecord_tlg_acs_agent_subsidiary', operator: 'anyof', values: driverSubsidiary
                        });
                    filters.push(filterOne);
                    var filterTwo = search.createFilter({ 
                        name: 'custrecord_tlg_acs_period', operator: 'anyof', values: periodID
                    });
                    ...
                    var filters = searchLoad.filters; 
                    var filterOne = search.createFilter({ 
                            name: 'subsidiary', operator: 'anyof', values: driverSubsidiary
                        });
                    filters.push(filterOne);
                    var filterTwo = search.createFilter({ 
                        name: 'custcol_tlg_acs_number', operator: 'contains', values: periodName
                    });
                    filters.push(filterTwo); 
                    ...
                    var filterOne = search.createFilter({ 
                            name: 'custrecord_tlg_hotel_comm_subsidiary', operator: 'anyof', values: driverSubsidiary
                        });
                    filters.push(filterOne);
                    var filterTwo = search.createFilter({ 
                        name: 'custrecord_tlg_hotel_comm_acs_number', operator: 'contains', values: periodName
                    });
                    filters.push(filterTwo); 
                ...
                var result = [];
                  while (true) {
                    var result_set = searchLoad.run().getRange({
                        start: startIndex,
                        end: endIndex
                    });
                    if (result_set.length > 0) {
                        result = result.concat(result_set);
                        startIndex = endIndex;
        ...
        function getMissingAgentCodesTransactions(missingAgentCode, driverSubsidiary){
            try{
                ...
                var result = [];
                  while (true) {
                    var result_set = transactionSearch.run().getRange({
                        start: startIndex,
                        end: endIndex
                    });
                    if (result_set.length > 0) {
                ...
                log.debug('result', 'getMissingAgentCodesTransactions result LENGTH: '+result.length);
               
                //log.debug('result', 'getMissingAgentCodesTransactions result: '+JSON.stringify(result));
        ...
        function checkInactiveAgents(results){
            try{
                ...
                var tranIDAdded = [];
                log.debug('results.length', 'FUNC checkInactiveAgents results.length: '+results.length);
                ...
                log.debug('tranIDAdded', 'tranIDAdded Length: '+tranIDAdded.length);
    
                log.debug('tranIDAdded', 'tranIDAdded: '+tranIDAdded);
    
                //log.debug('inactiveAgentTranIDs', 'After loop inactiveAgentTranIDs: '+inactiveAgentTranIDs);
